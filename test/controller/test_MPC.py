...
x_ref = np.zeros((DIStateIndices.dim, horizon+1))
u_ref = np.zeros((DIInputIndices.dim, horizon))
...
x_init = DISITFactory()
x_init = x_init.trajectory_from_numpy_array(traj_np=x_init_np,mode='state',time=time_state,t_0=0,delta_t=delta_t)
u_init_np = np.zeros((DIInputIndices.dim, horizon))
u_init = DISITFactory()
u_init = u_init.trajectory_from_numpy_array(traj_np=u_init_np,mode='input',time=time_state,t_0=0,delta_t=delta_t)
...
x_sol, u_sol, iteration_history = solver._solve_ocp(x0, x_ref_np=x_ref, u_ref_np=u_ref, x_init_np=x_init, u_init_np=u_init)
