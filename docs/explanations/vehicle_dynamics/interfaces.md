...
The [`StateInputDisturbanceTrajectoryFactoryInterface`](../../core_api/vehicle_dynamics/interfaces.md) (short `sidt_factory`) implements the functionality for instantiating these dataclass objects from `numpy` arrays or by providing the respective attributes as individual arguments.
...
For full-state feedback (see also the [sensor model documentation](../simulation/sensor_models.md)), each model defines a full-state noise vector:
