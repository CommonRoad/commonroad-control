...
- (combined) acceleration constraints: $\forall k \in \{0,\dots,N\}: a_{\mathrm{comb}}(x(k),u(k)) \leq 1$, where $a_{\mathrm{comb}}(x(k),u(k))$ returns the normalized combined acceleration, see the [vehicle models documentation](../vehicle_dynamics/interfaces.md).
...
The [`ModelPredictiveControl`](../../core_api/control/mpc.md) class implements the [`ControllerInterface`](../../core_api/control/controller_interface.md).
...
To accomodate different OCP solvers, every OCP solver has to implement the [`OptimalControlSolverInterface`](../../core_api/control/mpc.md).
The following parameters are required to setup an OCP solver:


| Parameter       | Variable type                                                                                 | Description                                                                                                                                                                               |
|-----------------|-----------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `vehicle_model` | [`VehicleModelInterface`](../../core_api/vehicle_dynamics/interfaces.md)                      | Vehicle model for predicting future states and computing the combined acceleration.                                                                                                       |
| `sidt_factory`  | [`StateInputDisturbanceTrajectoryFactoryInterface`](../../core_api/vehicle_dynamics/interfaces.md) | Required for converting the solution of the OCP to instances of state/input/trajectory dataclasses, see the [vehicle models documentation](../vehicle_dynamics/interfaces.md) |
| `horizon`       | `int`                                                                                         | Number of (discrete) time steps for prediction.                                                                                                                                           |
| `delta_t`       | `float`                                                                                       | Sampling time in seconds for the OCP.                                                                                                                                                     |
| `ocp_parameters` | [`OCPSolverParameters`](../../core_api/control/mpc.md)                                        | Provides parameters of the OCP solver set by the user, like the value of penalty weights for penalizing theviolation of softened constraitns.                                             |
...
The [`OptimalControlSCvx`](../../core_api/control/mpc.md) solver is based on the successive convexification algorithm proposed in 
