...
class ReferenceTrajectoryFactory:
    ...
    def get_reference_trajectory_at_time(
        self, t: float, consider_look_ahead: bool = True
        ...
        :param consider_look_ahead: boolean indicating whether the look-ahead time should be considered or not
        :return: state and input reference trajectory - TrajectoryInterface
        """

        # ... extract state reference trajectory
        t_0 = t + self._t_look_ahead if consider_look_ahead else t
        tmp_x_ref_points = [
            self._x_ref.get_point_at_time(time=t_0 + kk * self._dt_controller, sidt_factory=self._sidt_factory)
        ...
        tmp_u_ref_points = [
            self._u_ref.get_point_at_time(time=t_0 + kk * self._dt_controller, sidt_factory=self._sidt_factory)
